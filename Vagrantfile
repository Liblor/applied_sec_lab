...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
    ...
    master.each do |hostname, info|
        config.vm.define hostname do |hostconf|
            ...
            hostconf.vm.provision "shell", inline: <<-SHELL
                sudo su - #{ANSIBLE_UNAME} -c 'eval "$(ssh-agent -s)"; sshpass -v -P "Enter" -p $(cat /vagrant/ansible_passphrase.txt) ssh-add ~/.ssh/id_rsa; ansible-playbook -i production site.yml'
            SHELL
